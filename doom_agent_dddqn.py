...
class DoomAgentDDQN:
    def __init__(self,starting_eps,ending_eps,env,policy,target,total_steps,batch_size,capacity,device,gamma,optimizer,
                 update_target,update_policy,episodes,frames,update_eval,update_save):
        self.loss = nn.MSELoss()
        self.policy = policy
        self.policy_copy = policy
        self.target = target
    ...
    def update_weight(self):
        ...
        for sample in samples:
            ...
            if sample[3]!=None:
                action = torch.argmax(self.policy_copy(sample[3]))
                next_state_value = (self.target(sample[3])[action])
            else:
            ...
            if self.env.scenario == "basic" or self.env.scenario == "deadly_corridor":
                if self.memory.len() >= self.batch_size:
                    if self.steps % self.update_policy == 0:
                        self.update_weight()
                        self.policy_copy.load_state_dict(self.policy.state_dict())
            if done:
                if self.env.scenario == "defend_the_center":
                    if self.memory.len() >= self.batch_size:
                        self.update_weight()
